...
[Generator]
public sealed class JfSQLiteGenerator : IIncrementalGenerator
{
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        context.RegisterPostInitializationOutput(ctx => ctx.AddSource(
            "JfSQLiteGeneratorAttributes.g.cs",
    ...
    public static bool IsSyntaxTargetClassDec(SyntaxNode syntaxNode)
    {
        return
            ...
            cDeclarationSyntax.AttributeLists.Any(al => al.Attributes.Any(a => GeneratorAttributes._jfClassAttributes.Contains(a.Name.ToString())));
    }
    ...
    public static bool IsSyntaxTargetRecordDec(SyntaxNode syntaxNode)
    {
        return
            ...
            rDeclarationSyntax.AttributeLists.Any(al => al.Attributes.Any(a => GeneratorAttributes._jfClassAttributes.Contains(a.Name.ToString())));
    }
    ...
    public void Execute(Compilation compilation, ImmutableArray<ClassDeclarationSyntax> classes, SourceProductionContext context)
    {
        ...
        foreach (ClassDeclarationSyntax classSyntax in classes.Where(c => c.AttributeLists.Any(al => al.Attributes.Any(a => a.Name.ToString() == GeneratorAttributes._jfSQLiteTable))))
        {
        ...
        SourceProductionContext context)
    {
        ...
        foreach (RecordDeclarationSyntax recordSyntax in records.Where(c => c.AttributeLists.Any(al => al.Attributes.Any(a => a.Name.ToString() == GeneratorAttributes._jfSQLiteTable))))
        {
        ...
        JfGenCategory genCategory)
    {
        ...
        string tableName = symbolAttributeLookup.Contains(GeneratorAttributes._jfSQLiteTable)
            ? symbolAttributeLookup[GeneratorAttributes._jfSQLiteTable].First().ConstructorArguments.FirstOrDefault().Value?.ToString() ?? className
            : className;
        ...
        foreach (MemberDeclarationSyntax member in members)
        {
            ...
            bool isPrimaryKey = pds.AttributeLists.Any(al => al.Attributes.Any(a => a.Name.ToString() == GeneratorAttributes._jfSQLiteKey));
            if (isPrimaryKey)
            {
                pkColName = propName;
            ...
            {
                foreach (AttributeSyntax a in als.Attributes.Where(a => a.Name.ToString() == GeneratorAttributes._jfSQLiteForeignKey))
                {
        ...
        {
            if (!symbolAttributeLookup.Contains(GeneratorAttributes._jfSQLiteIndex))
            {
                yield break;
            }

            // generate any indexes
            foreach (AttributeData ad in symbolAttributeLookup[GeneratorAttributes._jfSQLiteIndex])
            {
        ...
        IEnumerable<string> getFnFilterParams(bool includeNullFilter)
        {
            foreach (TableColInfoRec rec in tableColInfo)
            {
                ...
                switch (rec.propType)
                {
                    ...
                    case "decimal":
                    case "double":
                        yield return $"JfSQLiteUtils.JfNumericOperatorCodes {rec.name}Operator = JfSQLiteUtils.JfNumericOperatorCodes.Equals";
                        break;
                }
        ...
        IEnumerable<string> getConditionLines(bool includeNullFilter, bool allowsOrJoining)
        {
            foreach (TableColInfoRec rec in tableColInfo)
            {
                ...
                switch (rec.propType)
                {
                    ...
                    case "decimal":
                    case "double":
                        conditionComparator = $$$"""{JfSQLiteUtils.GetSqlOperator({{{rec.name}}}Operator)}""";
                        break;
                }
